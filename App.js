...
const Percentage_1 = __importDefault(require("./components/Percentage"));
function App() {
    // All states variables.
    const [bill, setBill] = (0, react_1.useState)(0);
    const [percent, setPercent] = (0, react_1.useState)(0);
    const [custom, setCustom] = (0, react_1.useState)("");
    const [people, setPeople] = (0, react_1.useState)(1);
          ...
          <section>
            ...
            <div className="input-wrapper">
              <label className="input-icon">
                <bs_1.BsCurrencyDollar />
              </label>
              <label for="bill" className="sr-only">
                Bill
              </label>
              <input id="bill" className="inputs" type="number" placeholder="0" value={bill} onChange={handleMoney} min="0" max="99999"/>
            </div>
          </section>
          <section>
            <h2>Select Tip %</h2>
            <div className="tip-grid grid text-white fs-700">
              <Percentage_1.default id={5} handle={handlePercent}/>
              <Percentage_1.default id={10} handle={handlePercent}/>
              <Percentage_1.default id={15} handle={handlePercent}/>
              <Percentage_1.default id={25} handle={handlePercent}/>
              <Percentage_1.default id={50} handle={handlePercent}/>
              <label for="custom" className="sr-only">
                Custom percentage
              </label>
              <input id="custom" className="custom fs-600 fw-700" placeholder="Custom" type="number" value={custom} onChange={handleCustom} min="0"></input>
            </div>
          </section>
          <section>
            ...
            <div className="input-wrapper">
              ...
              <input id="people" className={style} type="number" onChange={handlePeople} value={people} min="0"/>
              <label className="input-icon">
                <bs_1.BsPersonFill />
              </label>
            </div>
